...
class StablePathNavigator:
    ...
    def waypoint_navigation_control(self):
        ...
        # 计算到当前路径点的距离和角度
        distance = self.calculate_distance_to_waypoint(current_waypoint)
        target_angle = self.calculate_angle_to_waypoint(current_waypoint)
        angle_error = self.calculate_angle_error(target_angle)
        
        # 记录日志（限制频率）
        rospy.loginfo_throttle(2, 
            f"路径点 {self.current_waypoint_index}/{len(self.waypoints)-1}: "
            f"距离={distance:.3f}m, 角度误差={math.degrees(angle_error):.1f}°")
